...
public class DefaultApplicationProvider implements ApplicationProvider {
    ...
    /**
     * 初始化appId的值(系统变量>环境变量>配置文件)
     * 
     * void
     * 
     * @date: 2020年4月27日 上午10:11:18
     */
    ...
    private void initAccessKey() {
        ...
        if (!Utils.isBlank(accessKeySecret)) {
            ...
            logger.info("ACCESSKEY SECRET is set by apollo.accesskey.secret property from System Property");
            return;
        }
        ...
        if (!Utils.isBlank(accessKeySecret)) {
            accessKeySecret = accessKeySecret.trim();
            logger.info("ACCESSKEY SECRET is set by APOLLO_ACCESSKEY_SECRET property from OS environment variable");
            return;
        }
