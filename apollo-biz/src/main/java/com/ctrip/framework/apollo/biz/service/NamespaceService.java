...
public class NamespaceService {
	...
	public Namespace findChildNamespace(String appId, String parentClusterName, String namespaceName) {
		...
		// the child namespace is the intersection of the child clusters and
		// child namespaces
		for (Namespace namespace : namespaces) {
	...
	/**
	 * 查找父namespace(先查询namespace对应cluster, 如果cluster有父cluster, 再根据父cluster查找父namespace)
	 * @param namespace
	 * @return
	 */
	...
	public Namespace deleteNamespace(Namespace namespace, String operator) {
		...
		// Child namespace releases should retain as long as the parent
		// namespace exists, because parent namespaces' release
		// histories need them
		if (!isChildNamespace(namespace)) {
		...
		if (childNamespace != null) {
			...
			// delete child namespace's releases. Notice: delete child namespace
			// will not delete child namespace's releases
		...
		// Publish release message to do some clean up in config service, such
		// as updating the cache
