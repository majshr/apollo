 ...
 * @author mengaijun
 * @Description: TODO
 * @date: 2020年4月23日 下午6:02:55
 */
@Configuration
public class AuthConfiguration {

    /**
     * spring.profiles.active = ctrip<br>
     * 实现方式1: 携程内部实现, 接入了SSO并实现用户搜索, 查询接口
     */
    @Configuration
    @Profile("ctrip")
    static class CtripAuthAutoConfiguration {
        ...
        public FilterRegistrationBean authenticationFilter() {
            ...
            // we don't want to use session to store login information, since we
            // will be deployed to a cluster, not a single instance
    ...
    /**
     * spring.profiles.active = auth<br>
     * 实现方式2: 使用apollo提供的spring security 简单认证
     */
    @Configuration
    @Profile("auth")
    static class SpringSecurityAuthAutoConfiguration {
        ...
        /**
         * Portal 页面如果长时间不刷新，登录信息会过期。通过此来刷新登录信息。
         * 
         * @return SsoHeartbeatHandler
         * @date: 2020年4月24日 上午10:22:33
         */
        @Bean
        ...
        public JdbcUserDetailsManager jdbcUserDetailsManager(AuthenticationManagerBuilder auth, DataSource datasource)
                throws Exception {
            JdbcUserDetailsManager jdbcUserDetailsManager = auth.jdbcAuthentication() // 基于
                                                                                      // JDBC
                    .passwordEncoder(new BCryptPasswordEncoder()) // 加密方式为
            ...
            jdbcUserDetailsManager
                    .setCreateAuthoritySql("insert into `Authorities` (Username, Authority) values (?,?)"); // 插入
                                                                                                            // Authorities
    ...
    /**
     * 注解@EnableWebSecurity ，禁用 Boot 的默认 Security 配置，配合 @Configuration
     * 启用自定义配置（需要继承 WebSecurityConfigurerAdapter ）。<br>
     * 注解@EnableGlobalMethodSecurity(prePostEnabled = true) ，启用 Security
     * 注解，例如最常用的 @PreAuthorize 。<br>
     * 
     * @author mengaijun
     * @Description: TODO
     * @date: 2020年4月24日 上午10:36:46
     */
    @Order(99)
    @Profile("auth")
    @Configuration
        ...
        protected void configure(HttpSecurity http) throws Exception {
            ...
            http.authorizeRequests()
                    .antMatchers("/prometheus/**", "/metrics/**", "/openapi/**", "/vendor/**", "/styles/**",
                            "/scripts/**", "/views/**", "/img/**", "/i18n/**", "/prefix-path")
                    .permitAll().antMatchers("/**").hasAnyRole(USER_ROLE); // 设置统一的
                                                                           // URL
                                                                           // 的权限校验，只判断是否为登陆用户。

            http.formLogin().loginPage("/signin").defaultSuccessUrl("/", true).permitAll().failureUrl("/signin?#/error")
                    .and().httpBasic(); // 登录页
    ...
    static class SpringSecurityLDAPConfigurer extends WebSecurityConfigurerAdapter {
        ...
        public SpringSecurityLDAPConfigurer(final LdapProperties ldapProperties,
                final LdapContextSource ldapContextSource, final LdapExtendProperties ldapExtendProperties) {
        ...
        public FilterBasedLdapUserSearch userSearch() {
            ...
            FilterLdapByGroupUserSearch filterLdapByGroupUserSearch = new FilterLdapByGroupUserSearch(
                    ...
                    ldapExtendProperties.getGroup().getGroupBase(), ldapContextSource,
                    ldapExtendProperties.getGroup().getGroupSearch(), ldapExtendProperties.getMapping().getRdnKey(),
        ...
        public LdapAuthenticationProvider ldapAuthProvider() {
            ...
            // Rewrite the logic of LdapAuthenticationProvider with
