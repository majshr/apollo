...
public class ItemService {
	...
	public ItemService(final UserInfoHolder userInfoHolder, final NamespaceAPI namespaceAPI, final ItemAPI itemAPI,
			final ReleaseAPI releaseAPI, final @Qualifier("fileTextResolver") ConfigTextResolver fileTextResolver,
	...
	public void updateConfigItemByText(NamespaceTextModel model) {
		...
		// 解析成 ItemChangeSets(oldItems通过数据库查找)
		...
		releaseItemDTOs.forEach((key, value) -> {
			...
			if (oldItem == null) {
				...
				changeSets.addCreateItem(
						buildNormalItem(0L, namespaceId, key, value, deletedItemDto.getComment(), lineNum.get()));
			} else if (!oldItem.getValue().equals(value) || lineNum.get() != oldItem.getLineNum()) {
				changeSets.addUpdateItem(
						buildNormalItem(oldItem.getId(), namespaceId, key, value, oldItem.getComment(), lineNum.get()));
			}
