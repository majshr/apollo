...
public class ConfigUtil {
    ...
    public ConfigUtil() {
        warnLogRateLimiter = RateLimiter.create(0.017); // 1 warning log output
                                                        // per minute
        initRefreshInterval();
        initConnectTimeout();
        initReadTimeout();
    ...
    /**
     * 是否本地模式, 通过env是否为Env.LOCAL判断
     * 
     * @return boolean
     * @date: 2020年4月27日 下午5:03:54
     */
    ...
    private void initPropertiesOrdered() {
        ...
        if (!Strings.isNullOrEmpty(enablePropertiesOrdered)) {
            try {
                ...
                logger.warn("Config for {} is invalid: {}, set default value: false", APOLLO_PROPERTY_ORDER_ENABLE,
